...
__host__ void generate_rand_data(unsigned int * data, unsigned int num_elements)
{
	...
	{
		data[i] = rand(); // [JRR] Generate random data
	}
}
				...
				const unsigned int tid)
{
	...
	data[tid] = (unsigned int) pow(data[tid], 2.0); // [JRR] Square the input data and replace the contents
}
...
__host__ float execute_register_memory_operations(void)
{
  ...
  cudaStreamCreate(&stream); 
	...
	cudaFree((void* ) d_data);		//[JRR] free device data
	cudaFreeHost(hi_data);	 			//[JRR] free up the host memory
	cudaFreeHost(hf_data);	 			//[JRR] free up the host memory
  cudaDeviceReset();            //[JRR] Reset the device
...
__host__ float execute_shared_memory_operations()
{
	...
	//set CUDA stream
  cudaStream_t stream;          // [JRR] Define Stream
  cudaStreamCreate(&stream);    // [JRR] Create Stream
...
int main(void) {
  ...
  //test harness for timing some kernels using streams and events
	float delta_shared   = execute_shared_memory_operations();   // [JRR] Added shared memory operations invocation
	float delta_register = execute_register_memory_operations(); // [JRR] Added register memory operations invocation
